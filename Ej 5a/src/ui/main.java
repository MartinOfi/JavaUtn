...
//		Esto debe ser realizado por otra clase que cree los objetos de las clases lógicas y les asigne los datos necesarios.
		...
		Empleado[] employeeList = employeeLoad(reader);
		show(employeeList);

	}

	private static void show(Empleado[] employeeList) {
	...
	private static Empleado[] employeeLoad(Scanner reader) {
		Empleado[] employeeList = new Empleado[3];
		for (int i = 0; i < 3; i++) {
